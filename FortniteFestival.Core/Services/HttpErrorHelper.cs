...
namespace FortniteFestival.Core.Services
{
    public static class HttpErrorHelper
    {
        ...
        private static readonly ConcurrentDictionary<string, int> _errorCodeCounts =
        ...
        public static (string errorCode, string errorMessage) ExtractError(string json)
        {
            ...
            try
            {
                using (var doc = JsonDocument.Parse(json))
                {
                    ...
                    string msg = null;
                    if (
                        root.TryGetProperty("errorCode", out var ecProp)
                        && ecProp.ValueKind == JsonValueKind.String
                    )
                        code = ecProp.GetString();
                    // Multiple patterns for message text
                    if (
                        root.TryGetProperty("errorMessage", out var emProp)
                        && emProp.ValueKind == JsonValueKind.String
                    )
                        msg = emProp.GetString();
                    else if (
                        root.TryGetProperty("message", out var mProp)
                        && mProp.ValueKind == JsonValueKind.String
                    )
                        msg = mProp.GetString();
                    if (!string.IsNullOrEmpty(code))
                        _errorCodeCounts.AddOrUpdate(code, 1, (_, v) => v + 1);
                    return (code, msg);
                }
            }
            catch
        ...
        )
        {
            ...
            string snippet =
                body == null ? "<no-body>" : body.Replace('\n', ' ').Replace('\r', ' ');
            ...
            if (!string.IsNullOrWhiteSpace(errorMessage))
                sb.Append(' ').Append("msg=\"").Append(Truncate(errorMessage, 120)).Append('"');
        ...
        {
            if (_errorCodeCounts.IsEmpty)
                return "ErrorCodeSummary: <none>";
            var parts = _errorCodeCounts
        ...
        public static string ComputeCorrelationId(Exception ex)
        {
            ...
            string sig =
                ...
                + ex.Message
                + "|"
                + (ex.StackTrace == null ? "" : new string(ex.StackTrace.Take(200).ToArray()));
            ...
            using (var sha = SHA256.Create())
            {
                ...
                for (int i = 0; i < hash.Length && i < 4; i++)
                    sb.Append(hash[i].ToString("X2")); // 8 hex chars
                return sb.ToString();
            }
        }
    }
