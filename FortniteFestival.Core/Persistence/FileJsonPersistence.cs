    ...
    public class FileJsonPersistence : IFestivalPersistence
    {
        ...
        public Task<IList<LeaderboardData>> LoadScoresAsync()
        {
            try
            {
                ...
                var list =
                    JsonConvert.DeserializeObject<List<LeaderboardData>>(json)
        ...
        public Task SaveScoresAsync(IEnumerable<LeaderboardData> scores)
        {
            try
            {
                File.WriteAllText(
                    _path,
                    JsonConvert.SerializeObject(scores.ToList(), Formatting.Indented)
                );
            }
            catch { }
        ...
        {
            return Task.FromResult<IList<Song>>(new List<Song>());
        }
