...
import {useCardGrid, Screen, getInstrumentStatusVisual, SortModal, FilterModal, FrostedSurface, FestivalTextInput, CenteredEmptyStateCard, PageHeader, HamburgerButton, SongRow as SongRowView, WIN_SCROLLBAR_INSET, FadeScrollView, Colors, Layout, Gap, Font, LineHeight, Radius, Opacity} from '@festival/ui';
import type {InstrumentChipVisual, InstrumentDetailData, SongRowDisplayData} from '@festival/ui';
...
const SongRow = React.memo<SongRowWrapperProps>(function SongRow(props) {
  ...
  const instrumentDetail = useMemo<InstrumentDetailData | undefined>(() => {
    ...
    if (!leaderboardData) return undefined;
    const tracker = (leaderboardData as any)[props.selectedInstrumentFilter];
    if (!tracker || !tracker.initialized) return undefined;
    return {
...
export function SongsScreen(props: {onOpenSong?: (songId: string, title: string) => void}) {
  ...
  const listContentStyle = useMemo(() => [styles.list, {paddingBottom: tabBarHeight, paddingRight: WIN_SCROLLBAR_INSET}], [tabBarHeight]);
  ...
  const sortIconColor = isSortActive ? Colors.accentBlue : Colors.textSecondary;
  const filterIconColor = isFilterActive ? Colors.accentBlue : Colors.textSecondary;

  return (
    <Screen>
      <View style={styles.content}>
        <PageHeader title="Songs" left={hamburger} />
...
const styles = StyleSheet.create({
  ...
  smallBtn: {
    ...
    paddingVertical: Platform.OS === 'ios' ? Gap.lg : Gap.md,
  },
  smallBtnPressed: {
