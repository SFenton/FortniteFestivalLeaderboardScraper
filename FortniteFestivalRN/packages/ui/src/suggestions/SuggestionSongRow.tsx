...
import {SongRowShell} from '../songs/SongRowShell';
...
import {Colors, Font, LineHeight, Gap, Opacity, Size} from '../theme';
...
export const SuggestionSongRow = React.memo(function SuggestionSongRow(props: {
  ...
  const metaText = useMemo(
    () => `${item.artist}${item.artist && year ? ' • ' : ''}${year ?? ''}`,
    [item.artist, year],
  );
  ...
  const rightContent = useMemo(() => (
    <View style={songRowStyles.songRight}>
      ...
      {hideRightSideCompletely ? null : isPercentileCategory && percentilePill ? (
        ...
        <View style={styles.songRightSingle}>
          <Text style={songRowStyles.songRightText}>{right}</Text>
        </View>
      ) : null}
    </View>
  ), [hideRightSideCompletely, showUnfcBadge, showRightInstrumentIcon, props.useCompactLayout, row, isPercentileCategory, percentilePill, rightInstrumentKeyFinal, unfcPercent, right]);

  const bottomContent = useMemo(() => {
    if (!starGainsStarsVisual) return undefined;
    return (
