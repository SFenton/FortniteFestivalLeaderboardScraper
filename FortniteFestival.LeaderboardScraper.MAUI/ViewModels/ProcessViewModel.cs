...
public class ProcessViewModel : BaseViewModel
{
    ...
    {
        get => _logJoined;
        private set => Set(ref _logJoined, value);
    }
    ...
    public string ExchangeCode
    {
        ...
        set
        {
            Set(ref _exchangeCode, value);
            _state.ExchangeCode = value;
            UpdateFetchEnabled();
        }
    }
    ...
    {
        get => _progressPct;
        private set => Set(ref _progressPct, value);
    }
    ...
    )
    {
        ...
        GenerateCodeCommand = new Command(() =>
            Launcher.OpenAsync(
                new Uri(
    ...
    private void EnqueueLog(string line)
    {
        lock (_logLock)
        {
            ...
            if (_pendingLogs.Count > 2000)
            { // prevent unbounded growth in memory
                while (_pendingLogs.Count > 1500)
    ...
    private void WireServiceEvents()
    {
        ...
        _service.SongProgress += (cur, total, title, started) =>
            MainThread.BeginInvokeOnMainThread(() => OnSongProgress(cur, total, title, started));
    ...
    private void CopyInstrumentSettings(Settings src, Settings dest)
    {
        if (src == null || dest == null)
            return;
        dest.QueryLead = src.QueryLead;
        dest.QueryDrums = src.QueryDrums;
        dest.QueryVocals = src.QueryVocals;
        dest.QueryBass = src.QueryBass;
        dest.QueryProLead = src.QueryProLead;
        dest.QueryProBass = src.QueryProBass;
        dest.DegreeOfParallelism = src.DegreeOfParallelism;
    }
    ...
    private async Task FetchAsync()
    {
        ...
        ProgressPct = 0;
        ProgressLabel = "0%";
        EnqueueLog("Starting score fetch...");
    ...
    {
        if (total > 0)
            _state.ProgressTotal = total;
        _state.ProgressCurrent = current;
        if (_state.ProgressTotal > 0)
        {
            double pct = (double)_state.ProgressCurrent / _state.ProgressTotal * 100.0;
            ProgressPct = pct;
    ...
    private void UpdateFetchEnabled() =>
        FetchEnabled =
    ...
    private void UpdateMetrics()
    {
        ...
        Metrics =
            $"Req:{requests} Improved:{improved} Empty:{empty} Errors:{errors} Bytes:{bytes} Elapsed:{elapsed:0.0}s";
    }
}
