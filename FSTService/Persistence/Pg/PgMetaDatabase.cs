...
public sealed class PgMetaDatabase : IMetaDatabase
{
        ...
        int? seasonRank, int? allTimeRank, int? difficulty)
    {
        ...
        cmd.Parameters.AddWithValue("scoreAchievedAt", scoreAchievedAt is not null ? DateTime.Parse(scoreAchievedAt) : DBNull.Value);
    ...
    public int InsertScoreChanges(IReadOnlyList<ScoreChangeRecord> changes)
    {
        ...
        foreach (var c in changes)
        {
            ...
            pScoreAchievedAt.Value = c.ScoreAchievedAt is not null ? DateTime.Parse(c.ScoreAchievedAt) : DBNull.Value;
    ...
    public List<string> GetOrphanedRegisteredAccounts() { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "SELECT DISTINCT ru.account_id FROM registered_users ru JOIN account_names an ON an.account_id = ru.account_id WHERE NOT EXISTS (SELECT 1 FROM user_sessions us WHERE us.username = an.display_name AND us.revoked_at IS NULL AND us.expires_at > @now) AND EXISTS (SELECT 1 FROM user_sessions us WHERE us.username = an.display_name)"; cmd.Parameters.AddWithValue("now", DateTime.UtcNow); var list = new List<string>(); using var r = cmd.ExecuteReader(); while (r.Read()) list.Add(r.GetString(0)); return list; }
    ...
    public List<(string DeviceId, string AccountId)> GetDeviceAccountMappings() { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "SELECT device_id, account_id FROM registered_users"; var list = new List<(string, string)>(); using var r = cmd.ExecuteReader(); while (r.Read()) list.Add((r.GetString(0), r.GetString(1))); return list; }
    ...
    public long InsertSession(string username, string deviceId, string refreshTokenHash, string? platform, DateTime expiresAt) { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "INSERT INTO user_sessions (username, device_id, refresh_token_hash, platform, issued_at, expires_at) VALUES (@username, @deviceId, @hash, @platform, @now, @expiresAt) RETURNING id"; cmd.Parameters.AddWithValue("username", username); cmd.Parameters.AddWithValue("deviceId", deviceId); cmd.Parameters.AddWithValue("hash", refreshTokenHash); cmd.Parameters.AddWithValue("platform", (object?)platform ?? DBNull.Value); cmd.Parameters.AddWithValue("now", DateTime.UtcNow); cmd.Parameters.AddWithValue("expiresAt", expiresAt); return (long)(int)cmd.ExecuteScalar()!; }
    public UserSessionInfo? GetActiveSession(string refreshTokenHash) { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "SELECT id, username, device_id, platform, issued_at, expires_at FROM user_sessions WHERE refresh_token_hash = @hash AND revoked_at IS NULL AND expires_at > @now LIMIT 1"; cmd.Parameters.AddWithValue("hash", refreshTokenHash); cmd.Parameters.AddWithValue("now", DateTime.UtcNow); using var r = cmd.ExecuteReader(); if (!r.Read()) return null; return new UserSessionInfo { Id = r.GetInt32(0), Username = r.GetString(1), DeviceId = r.GetString(2), Platform = r.IsDBNull(3) ? null : r.GetString(3), IssuedAt = r.GetDateTime(4), ExpiresAt = r.GetDateTime(5) }; }
    public void RevokeSession(string refreshTokenHash) { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "UPDATE user_sessions SET revoked_at = @now WHERE refresh_token_hash = @hash AND revoked_at IS NULL"; cmd.Parameters.AddWithValue("hash", refreshTokenHash); cmd.Parameters.AddWithValue("now", DateTime.UtcNow); cmd.ExecuteNonQuery(); }
    public void RevokeAllSessions(string username) { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "UPDATE user_sessions SET revoked_at = @now WHERE username = @username AND revoked_at IS NULL"; cmd.Parameters.AddWithValue("username", username); cmd.Parameters.AddWithValue("now", DateTime.UtcNow); cmd.ExecuteNonQuery(); }
    public int CleanupExpiredSessions(DateTime cutoff) { using var conn = _ds.OpenConnection(); using var cmd = conn.CreateCommand(); cmd.CommandText = "DELETE FROM user_sessions WHERE (revoked_at IS NOT NULL AND revoked_at < @cutoff) OR (expires_at < @cutoff)"; cmd.Parameters.AddWithValue("cutoff", cutoff); return cmd.ExecuteNonQuery(); }
