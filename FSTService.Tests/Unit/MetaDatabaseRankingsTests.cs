...
public sealed class MetaDatabaseRankingsTests : IDisposable
{
    ...
    public void BandRankHistoryV2Schema_DoesNotRecreateRetiredLatestSnapshotLookupIndexes()
    {
        ...
        cmd.CommandText = """
            SELECT
                to_regclass('public.ix_btrhlv2_snapshot') IS NULL,
                to_regclass('public.band_team_rank_history_latest_v2_duet_snapshot_id_band_type_idx') IS NULL,
                to_regclass('public.band_team_rank_history_latest_v2_trio_snapshot_id_band_type_idx') IS NULL,
                to_regclass('public.band_team_rank_history_latest_v2_quad_snapshot_id_band_type_idx') IS NULL
            """;
    ...
    [Fact]
    public void RetiredBandRankHistoryLatestSnapshotLookupIndexMaintenanceSqlIsValid()
    {
        var today = DateOnly.FromDateTime(DateTime.UtcNow);
        _ = Db.GetBandRankHistoryV2Parity("Band_Duets", today);
        ...
        {
            """
            CREATE INDEX CONCURRENTLY band_team_rank_history_latest_v2_duet_snapshot_id_band_type_idx
                ON public.band_team_rank_history_latest_v2_duets USING btree (snapshot_id, band_type)
            """,
            """
            CREATE INDEX CONCURRENTLY band_team_rank_history_latest_v2_trio_snapshot_id_band_type_idx
                ON public.band_team_rank_history_latest_v2_trios USING btree (snapshot_id, band_type)
            """,
            """
            CREATE INDEX CONCURRENTLY band_team_rank_history_latest_v2_quad_snapshot_id_band_type_idx
                ON public.band_team_rank_history_latest_v2_quad USING btree (snapshot_id, band_type)
            """,
            """
            CREATE INDEX ix_btrhlv2_snapshot
                ON ONLY public.band_team_rank_history_latest_v2 USING btree (snapshot_id, band_type)
            """,
            """
            ALTER INDEX public.ix_btrhlv2_snapshot
                ATTACH PARTITION public.band_team_rank_history_latest_v2_duet_snapshot_id_band_type_idx
            """,
            """
            ALTER INDEX public.ix_btrhlv2_snapshot
                ATTACH PARTITION public.band_team_rank_history_latest_v2_trio_snapshot_id_band_type_idx
            """,
            """
            ALTER INDEX public.ix_btrhlv2_snapshot
                ATTACH PARTITION public.band_team_rank_history_latest_v2_quad_snapshot_id_band_type_idx
            """,
        })
        {
        ...
        using (var verify = conn.CreateCommand())
        {
            verify.CommandText = """
                ...
                FROM pg_index
                WHERE indexrelid IN (
                    'public.ix_btrhlv2_snapshot'::regclass,
                    'public.band_team_rank_history_latest_v2_duet_snapshot_id_band_type_idx'::regclass,
                    'public.band_team_rank_history_latest_v2_trio_snapshot_id_band_type_idx'::regclass,
                    'public.band_team_rank_history_latest_v2_quad_snapshot_id_band_type_idx'::regclass)
                  AND indisvalid
                  AND indisready
                """;
        ...
        {
            drop.CommandText = "DROP INDEX public.ix_btrhlv2_snapshot";
    ...
    [Fact]
    public void GetBandRankHistory_CompactV3DuetsReadsReadyProjectionWhenV2RowsAreGone()
    {
        SeedBandRankingsSource();
        ...
        using var v2Db = CreateMetaDatabase(BandRankHistoryApiReadSource.V2NarrowOnly);
        var expectedOverall = v2Db.GetBandRankHistory("Band_Duets", "p1:p2");
        var comboId = BandComboIds.FromEpicRawCombo("0:1");
        var expectedCombo = v2Db.GetBandRankHistory("Band_Duets", "p1:p2", comboId);
        SeedCompactV3Duets();

        using (var conn = _fixture.DataSource.OpenConnection())
        using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = "DELETE FROM band_team_rank_history_points_v2 WHERE band_type = 'Band_Duets'";
            cmd.ExecuteNonQuery();
        }
        ...
        var actualOverall = compactDb.GetBandRankHistory("Band_Duets", "p1:p2");
        var actualCombo = compactDb.GetBandRankHistory("Band_Duets", "p1:p2", comboId);
    ...
    public void GetBandRankHistory_CompactV3TriosReadsReadyProjectionWhenV2RowsAreGone()
    {
        ...
        persistence.UpsertBandEntries("song_0", "Band_Trios",
        [
            MakeBandEntry(["p1", "p2", "p3"], "0:1:3", 1000, isFullCombo: true),
            MakeBandEntry(["p4", "p5", "p6"], "0:1:3", 900),
        ]);
        persistence.UpsertBandEntries("song_1", "Band_Trios",
        [
            MakeBandEntry(["p1", "p2", "p3"], "0:1:3", 1200),
            MakeBandEntry(["p4", "p5", "p6"], "0:1:3", 1100, isFullCombo: true),
        ]);

        Db.RebuildBandTeamRankings("Band_Trios", totalChartedSongs: 2);
        Db.SnapshotBandRankHistoryChunked("Band_Trios", new BandRankHistorySnapshotOptions
        {
            WriteMode = BandRankHistoryWriteMode.V2Only,
        });

        using var v2Db = CreateMetaDatabase(BandRankHistoryApiReadSource.V2NarrowOnly);
        var expectedOverall = v2Db.GetBandRankHistory("Band_Trios", "p1:p2:p3");
        var comboId = BandComboIds.FromEpicRawCombo("0:1:3");
        var expectedCombo = v2Db.GetBandRankHistory("Band_Trios", "p1:p2:p3", comboId);
        SeedCompactV3Trios();

        using (var conn = _fixture.DataSource.OpenConnection())
        using (var cmd = conn.CreateCommand())
        {
            cmd.CommandText = "DELETE FROM band_team_rank_history_points_v2 WHERE band_type = 'Band_Trios'";
            cmd.ExecuteNonQuery();
        }

        using var compactDb = CreateMetaDatabase(
            BandRankHistoryApiReadSource.V2NarrowOnly,
            compactV3TriosReadEnabled: true);
        var actualOverall = compactDb.GetBandRankHistory("Band_Trios", "p1:p2:p3");
        var actualCombo = compactDb.GetBandRankHistory("Band_Trios", "p1:p2:p3", comboId);
    ...
    private void SeedCompactV3Duets()
    {
        using var conn = _fixture.DataSource.OpenConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            ...
            CREATE TABLE band_rank_history_combo_v3_duets (
                combo_ref INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                combo_id TEXT NOT NULL UNIQUE
            );
            CREATE TABLE band_team_rank_history_points_v3_duets (
            ...
            INSERT INTO band_rank_history_team_v3_duets (team_key)
            SELECT DISTINCT team_key
            FROM band_team_rank_history_points_v2
            WHERE band_type = 'Band_Duets';

            INSERT INTO band_rank_history_combo_v3_duets (combo_id)
            SELECT DISTINCT combo_id
            FROM band_team_rank_history_points_v2
            WHERE band_type = 'Band_Duets'
              AND combo_id <> '';

            INSERT INTO band_team_rank_history_points_v3_duets (
            ...
            JOIN band_rank_history_team_v3_duets team USING (team_key)
            LEFT JOIN band_rank_history_combo_v3_duets combo
    ...
    private void SeedCompactV3Trios()
    {
        using var conn = _fixture.DataSource.OpenConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            ...
            CREATE TABLE band_rank_history_combo_v3_trios (
                combo_ref INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                combo_id TEXT NOT NULL UNIQUE
            );
            CREATE TABLE band_team_rank_history_points_v3_trios (
                team_id BIGINT NOT NULL,
                scope_id SMALLINT NOT NULL,
                combo_ref INT NOT NULL,
                snapshot_date DATE NOT NULL,
            ...
            INSERT INTO band_rank_history_team_v3_trios (team_key)
            SELECT DISTINCT team_key
            FROM band_team_rank_history_points_v2
            WHERE band_type = 'Band_Trios';

            INSERT INTO band_rank_history_combo_v3_trios (combo_id)
            SELECT DISTINCT combo_id
            FROM band_team_rank_history_points_v2
            WHERE band_type = 'Band_Trios'
              AND combo_id <> '';

            INSERT INTO band_team_rank_history_points_v3_trios (
                team_id, scope_id, combo_ref, snapshot_date, snapshot_id, generation_id,
                snapshot_taken_at, row_fingerprint, adjusted_skill_rank, weighted_rank,
                fc_rate_rank, total_score_rank, adjusted_skill_rating, weighted_rating,
                fc_rate, total_score, songs_played, coverage, full_combo_count,
            ...
            SELECT
                ...
                points.raw_skill_rating
            FROM band_team_rank_history_points_v2 points
            JOIN band_rank_history_team_v3_trios team USING (team_key)
            LEFT JOIN band_rank_history_combo_v3_trios combo
              ON combo.combo_id = points.combo_id
             AND points.combo_id <> ''
