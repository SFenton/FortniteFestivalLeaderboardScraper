...
public sealed class ImprovementNotificationRecoveryServiceTests : IDisposable
{
    ...
    [Fact]
    public async Task RunPublishedScrapeAsync_ExplicitMaintenanceRebaselineReopensCompletedMarker()
    {
        var scrapeId = _fixture.Db.StartScrapeRun();
        _fixture.Db.CompleteScrapeRun(scrapeId, 1, 10, 1, 100);
        _fixture.Db.PublishScrapeRun(
        ...
        var notificationService = new ImprovementNotificationService(
            _fixture.DataSource,
            NullLogger<ImprovementNotificationService>.Instance);
        var options = Options.Create(new ImprovementNotificationOptions
        {
            Enabled = true,
            IncludePlayers = true,
            IncludeBands = false,
        ...
        await recovery.RunPublishedScrapeAsync(
            ...
            projectionScopes: [],
            force: false,
            source: "test-initial-complete",
            CancellationToken.None);
        Assert.Equal(
            "completed",
            notificationService.GetPublicationStatus().MarkerStatus);

        var baseline = await recovery.RunPublishedScrapeAsync(
            expectedPublishedScrapeId: scrapeId,
            execute: true,
            baselineOnly: true,
            refreshSoloProjection: false,
            projectionScopes: [],
            force: true,
            source: "test-maintenance-rebaseline",
            CancellationToken.None,
            reopenCompletedForMaintenance: true);

        Assert.True(baseline.BaselineOnly);
        Assert.Equal(
            "pending",
            notificationService.GetPublicationStatus().MarkerStatus);
        ...
        await recovery.RunPublishedScrapeAsync(
            ...
            projectionScopes: [],
            force: false,
            source: "test-post-maintenance-complete",
            CancellationToken.None);
        Assert.Equal(
            "completed",
            notificationService.GetPublicationStatus().MarkerStatus);
