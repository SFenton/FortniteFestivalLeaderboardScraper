...
import { Layout, MaxWidth, Gap, Colors, Font, Weight, ZIndex, Overflow, flexColumn, flexRow, flexCenter, CssValue, Position, Display, Align, BoxSizing } from '@festival/theme';
...
export const appStyles = {
  ...
  /** Sizing box that clips content between header and bottom nav. Actual scrolling is per-page. */
  scrollContainer: { flex: 1, overflow: Overflow.hidden, minHeight: 0 } as CSSProperties,
  ...
  nav: { display: Display.flex, alignItems: Align.center, gap: Gap.xl, padding: `${Layout.paddingTop}px ${Layout.paddingHorizontal}px ${Gap.md}px`, maxWidth: MaxWidth.card, margin: CssValue.marginCenter, width: CssValue.full, boxSizing: BoxSizing.borderBox, backgroundColor: 'transparent', flexShrink: 0, zIndex: ZIndex.popover, touchAction: 'none' } as CSSProperties,
  ...
  content: { flex: 1, ...flexColumn, position: Position.relative, minHeight: 0 } as CSSProperties,
