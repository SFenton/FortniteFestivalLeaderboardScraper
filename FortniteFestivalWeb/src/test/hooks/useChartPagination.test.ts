...
describe('useChartPagination', () => {
  ...
  it('shows all data when maxBars >= data length', () => {
    const { result } = renderHook(() => useChartPagination(data, 20, 'guitar'));
  ...
  it('slices to maxBars when data exceeds limit', () => {
    const { result } = renderHook(() => useChartPagination(data, 5, 'guitar'));
  ...
  it('setSelectedPoint updates selectedIndex', () => {
    const { result } = renderHook(() => useChartPagination(data, 10, 'guitar'));
  ...
  it('resets offset when instrument changes', () => {
    const { result, rerender } = renderHook(
      ({ inst }: { inst: string }) => useChartPagination(data, 5, inst),
  ...
  it('resets selectedPoint when instrument changes', () => {
    const { result, rerender } = renderHook(
      ({ inst }: { inst: string }) => useChartPagination(data, 10, inst),
      { initialProps: { inst: 'guitar' } },
    );
  ...
  it('handles empty data', () => {
    const { result } = renderHook(() => useChartPagination([], 5, 'guitar'));
  ...
  it('maxOffset and clampedOffset are correct', () => {
    const { result } = renderHook(() => useChartPagination(data, 3, 'guitar'));
