...
type UseBandInstrumentFilterControllerProps = {
  visible: boolean;
...
export function useBandInstrumentFilterController({
  visible,
  selectedBand,
  appliedAssignments,
  onApply,
  onReset,
}: UseBandInstrumentFilterControllerProps) {
  ...
  const handleSelect = useCallback((index: number, instrument: ServerInstrumentKey | null) => {
    ...
    setDraft(nextDraft);
  }, [configurations, draft, hasConfigurationData]);
  ...
  const dismissInvalidSelection = useCallback(() => {
    setPendingInvalidSelection(null);
  }, []);
  ...
  const apply = useCallback(() => {
    ...
    if (applyDisabled || !matchingConfiguration) return false;
    ...
    return true;
  }, [applyDisabled, configurations, draft, draftClearsAppliedFilter, hasChanges, matchingConfiguration, members, onApply, onReset]);
  ...
  return {
    ...
    hasError: !!detailQuery.error,
    availableInstruments,
    instrumentStates,
    pendingInvalidSelection,
    handleSelect,
    confirmInvalidSelection,
    dismissInvalidSelection,
    resetDraft,
    apply,
  };
}

export type BandInstrumentFilterController = ReturnType<typeof useBandInstrumentFilterController>;

export function BandInstrumentFilterPicker({ controller, compact }: { controller: BandInstrumentFilterController; compact: boolean }) {
  const { t } = useTranslation();
  const styles = useStyles();

  if (controller.isLoading && controller.members.length === 0) {
    return (
      <ModalSection>
    ...
    <>
      {controller.members.map((member, index) => (
        <InstrumentSlotSection
          ...
          onSelect={(instrument) => controller.handleSelect(index, instrument)}
          availableInstruments={controller.availableInstruments}
...
export function BandInstrumentFilterInvalidSelectionAlert({ controller }: { controller: BandInstrumentFilterController }) {
  const { t } = useTranslation();

  if (!controller.pendingInvalidSelection) return null;
  ...
  return (
    <ConfirmAlert
      ...
      onNo={controller.dismissInvalidSelection}
      onYes={controller.confirmInvalidSelection}
      onExitComplete={controller.dismissInvalidSelection}
    />
  );
}
...
export function areBandInstrumentDraftsEqual(a: readonly (ServerInstrumentKey | null)[], b: readonly (ServerInstrumentKey | null)[]) {
