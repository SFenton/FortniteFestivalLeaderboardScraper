...
/**
 * First-run demo: Rivals above and below the player.
...
export default function CompeteRivalsDemo() {
  const h = useSlideHeight();
  const s = useStyles();

  const budget = h || 320;
  const totalRows = Math.max(2, Math.floor((budget - 2 * LABEL_HEIGHT - Gap.lg) / (RIVAL_ROW_HEIGHT + 2)));
