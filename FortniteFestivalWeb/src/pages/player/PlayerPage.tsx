...
export default function PlayerPage({ accountId: propAccountId }: { accountId?: string } = {}) {
  ...
  const { isTracked: localIsTracked, syncStatusLoaded: localSyncStatusLoaded, isSyncing: localSyncing, phase: localPhase, backfillProgress: localBfProg, historyProgress: localHrProg, rivalsProgress: localRvProg, entriesFound: localEntriesFound, itemsCompleted: localItemsCompleted, totalItems: localTotalItems, currentSongName: localCurrentSong, seasonsQueried: localSeasonsQueried, rivalsFound: localRivalsFound, isThrottled: localIsThrottled, throttleStatusKey: localThrottleStatusKey, pendingRankUpdate: localPendingRankUpdate, estimatedRankUpdateMinutes: localEstimatedRankUpdateMinutes, probeStatusKey: localProbeStatusKey, nextRetrySeconds: localNextRetrySeconds, justCompleted: localJustCompleted, clearCompleted: localClearCompleted } =
    useSyncStatus(!isTrackedPlayer ? accountId : undefined, { track: false, useWebSocket: false });
  const showUntrackedHistoryNotice = !isTrackedPlayer && localSyncStatusLoaded && !localIsTracked;
