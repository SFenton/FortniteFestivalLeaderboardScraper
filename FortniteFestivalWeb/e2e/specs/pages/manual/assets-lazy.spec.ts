...
test('desktop Manual loads only near responsive images and preserves carousel state while scrolling', async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== WIDE_PROJECT, 'desktop Manual waterfall is covered at its production capture width');
  ...
  await expect.poll(() => firstImage.evaluate(image => {
  ...
  expect(await firstImage.evaluate(image => (image as HTMLImageElement).currentSrc)).toMatch(/navigation-overview-mobile-[a-f0-9]{12}-390\.webp$/);
  ...
  await expect.poll(() => compactImage.evaluate(image => (image as HTMLImageElement).currentSrc)).toMatch(/navigation-overview-compact-[a-f0-9]{12}-768\.webp$/);
...
test('mobile Manual selects the small source and keeps swipe/button behavior without eager far images', async ({ page }, testInfo) => {
  test.skip(!isPrimaryMobileProject(testInfo.project.name), 'mobile Manual waterfall is covered once');
  const manualRequests: string[] = [];
  const consoleErrors: string[] = [];
  const pageErrors: string[] = [];
  page.on('request', (request) => {
  ...
  await expect.poll(() => compactImage.evaluate(image => (image as HTMLImageElement).currentSrc)).toMatch(/navigation-overview-compact-[a-f0-9]{12}-(480|768|1024)\.webp$/);
