...
// Verifies shared Songs/Shop storage and request ownership.
  ...
  await page.addInitScript(({ profile, songId }) => {
    ...
    JSON.parse = function parseWithSongsCount(text: string, reviver?: (this: unknown, key: string, value: unknown) => unknown) {
      if (typeof text === 'string' && text.includes('"web23Marker":true')) {
        (window as unknown as Window & { __web23SongsParseCount: number }).__web23SongsParseCount += 1;
      }
      return parse.call(this, text, reviver);
    };
    Object.assign(window, { __web23LongTasks: [], __web23UnhandledRejections: [] });
    window.addEventListener('unhandledrejection', event => {
      (window as unknown as Window & { __web23UnhandledRejections: string[] }).__web23UnhandledRejections.push(String(event.reason));
    });
    if ('PerformanceObserver' in window) {
      try {
        const observer = new PerformanceObserver(list => {
          (window as unknown as Window & { __web23LongTasks: PerformanceEntry[] }).__web23LongTasks.push(...list.getEntries());
        });
