...
test.describe('Notification seen state', () => {
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    await seedAppState(page, isMobileProject(testInfo.project.name));
  });

  test('mobile badge and unread dots follow visibility-based seen state', async ({ page }, testInfo) => {
    test.skip(!isPrimaryMobileProject(testInfo.project.name), 'mobile-only notification shell behavior');
  ...
  test('desktop header notifications button opens the notifications modal', async ({ page }, testInfo) => {
    test.skip(!isPrimaryDesktopProject(testInfo.project.name), 'desktop header behavior is covered by the desktop project');
  ...
  test('player notification badge and modal rows respect hidden instruments', async ({ page }, testInfo) => {
    await seedPlayerNotificationFilterState(page, isMobileProject(testInfo.project.name));
    await page.goto('/#/songs', { waitUntil: 'load' });
    await dismissFirstRunIfVisible(page);

    const headerPrefix = isMobileProject(testInfo.project.name) ? 'mobile-header' : 'desktop-header';
