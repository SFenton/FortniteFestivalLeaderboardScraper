...
| Next implementation phase | Create immutable source cuts | Version catalog, shop, path, overlay, history, names, and cache surfaces; replace every `legacy_live_unversioned`/inherited binding with a ready generation-addressable binding before enabling request pinning. |
