...
| Bound physical snapshot generations | All nine instrument roots are generation-partitioned. Archive-only, quarantine/reattach, exact non-cascading DROP, logical restore, and H6 continuation/finalization are live-accepted. Candidate scrape `1345` and official scrape `1346` closed the DROP tier; cycle `33` then proved a 3.28 GiB Solo Guitar child through exact archive/network-none restore without source mutation. A separate default-off host control plane persists bounded immutable policy and largest-first plan evidence only. The offline reporter's source-preserving startup repair is committed, but a deployment retry rejected cumulative registration counters despite unchanged captured rows/schema/source/path state. Those asynchronously flushed deltas do not establish initializer DML and remain unattributed. The version-2 fresh-session proof now combines an empty DML allowlist with exact non-retention table identities and explicit unknown COMMIT acknowledgement; these closures require independent review. The compatible additive schema remains deployed and Solo Bass `1308` remains protected. | Review the combined proof and uncertainty handling, then use a natural idle stop and dedicated retention-only initialization with zero transaction DML, unchanged table identities and acknowledged commit plus exact row/schema/source/path/control/public parity and lock/resource gates before candidate service and guarded compatible worker acceptance. Null/uncertain committed state never authorizes retry or deployment. Treat cumulative counter drift as telemetry only; unexplained actual state drift still rejects. Require mutation readiness, genuine receipt and bounded offline current-cycle/oracle agreement, then plan/reconcile evidence. Do not roll back additive schema or claim the earlier binding mutation was repaired. Archive execution remains blocked on exact container binding, full-duration admission and cooperative process/Docker cleanup; automatic deletion, multi-child execution and sparse compaction remain separately gated |
