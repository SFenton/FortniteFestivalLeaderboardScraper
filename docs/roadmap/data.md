...
| Promote freeze-safe publication API cache candidate | PR #55 remains unmerged/undeployed. The corrected candidate requires exactly one canonical `PublicationBound` classification before cache admission, retains deny-list defense-in-depth, adds exact body/header/revision/alias/concurrency/staging/redaction evidence, and keeps only ten lazy overview variants. Refreshed L2/L1/write p95 is `3.273/0.313/11.149 ms`; every lazy live compute p95 is `6.707-11.114 ms`. Stratified eager logical payload is 19.02 MB/publication with 20.74 MB retained as the conservative upper bound. | New correction commit and green CI, focused review, then full-scrape candidate cache-key/JSON/ETag/publication parity, same-publication maintenance freeze injection, no >10% protected API or precompute/WAL/resource regression, rollback image/config, and separate promotion |
