...
| Execute the exact pro-bass rewrite when all gates pass | PR #57 protects current/previous/working physical source IDs, excludes stale unnamed maps, and hardcodes only `leaderboard_entries_snapshot_pro_bass`. The live archive/restore proves 308,536,699 rows, 125 IDs and exact catalog, then cleans 130,771,858,177 bytes of restore PGDATA. Production planning now uses loose-index IDs, metadata ownership, protected-only fingerprints and the verified archive input. The scratch build has 2,110,309,380 bytes of projected 4 TB margin, but rollback-safe pre-drop repatriation requires 66,575,033,638 bytes and is still 575,033,638 short at the 66 GB assumption. No live replacement/rewrite/swap/drop ran. | Merge/review the candidate; recover at least the repatriation shortfall; add and validate the exact production-owned scratch bind mount in a separate PostgreSQL recreate window; run fresh idle/unfrozen preflight and live-scrape parity; require source/API/reference parity while the old relation remains; run `repatriate` to return the accepted relation/catalog to `pg_default` and remove every scratch relation/tablespace; only then explicitly approve final old-relation drop. Retain the verified archive until a separate product-retention decision. |
