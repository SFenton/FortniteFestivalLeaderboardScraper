...
| Promote freeze-safe publication API cache candidate | PR #55 remains unmerged/undeployed. The bounded service-only A/B rejected approved head `5a227954`: exact Unicode body/ETag parity failed and five protected routes exceeded the matched 10% relative p95 gate (`+13.1%` to `+109.5%`) despite all absolute p95 values remaining under 9 ms. Baseline image/cache were restored. Classification, selected-context isolation, freeze simulation, transaction atomicity, restart L2, and resource gates passed. Live staging measured 15,574 rows, 24.45 MB physical DB growth, 409.98 MB peak free-space excursion, 466.46 MB WAL, zero temp bytes, and 167.94 s core precompute. Repository repair centralizes endpoint/precompute/projection encoding and reduces warm L1 safety work to one authoritative combined publication/freeze/failure snapshot without weakening publication identity. | Green CI, then repeat bounded service-only byte/ETag and predeclared 100-sample relative-p95 A/B. Merge remains blocked by the failed gates. Worker publication-switch verification remains deferred to the next natural scrape; no full scrape is required for the service retry. |
