> [!IMPORTANT]
> **Archived execution journal.** Completed, rejected, and future work are
> mixed below. Use `docs/roadmap/data.md` for current priorities.
...
- LOGICAL-RETIRE initially identified `141,462,937,600` bytes in the disabled
  `leaderboard_current_entries*` and `leaderboard_entry_versions*` shadow.
  After secondary-index retirement, scrape `1267` cleared the live parity gate
  and the separate 2026-07-28 maintenance phase truncated the remaining
  `123,173,888,000`-byte heap/primary-key family without `CASCADE`. The empty
  schemas, 20 primary keys, and 108-row metrics table remain.
...
- POST-1265-LOW-SCRATCH then retired only the four non-constraint secondary
  ...
  state changed.
- PROXY-RETUNE scrape `1266` completed `8,232/8,232` manifests and improved
...
- The refreshed P9 evaluation retains legacy `leaderboard_entries_*`:
  ...
  checksum.
- P9 now has a default-off worker reader candidate. When enabled, active
  current-state helpers, projection rebuilds, band extraction, precompute
  probes, and improvement-notification projection scans resolve finalized
  snapshots plus overlays and reject legacy-only scopes. Supplemental writes
  remain enabled and no production flag changed; complete scrape parity and
  performance are still required before writer-off or reclaim.
  Projection provenance is explicit through
  `solo_current_projection_scope.source_kind`; startup recovery rebuilds or
  prunes unresolved scopes before notifications, while player-profile
  precompute uses bounded bulk reads and only trusts a provenance-validated
  projection.
  Band extraction no longer depends on full snapshots retaining sparse band
  JSON: a narrow `leaderboard_band_context` accumulator is seeded once behind
  the candidate flag and maintained by both scrape and supplemental writers
...
The separate LOGICAL-RETIRE maintenance phase then reclaimed
`123,173,593,088` database bytes from the disabled logical current/version
shadow. The final scrape guard sampled about `164.33 GB` free, leaving more
than `103.9 GB` above the corrected one-run requirement. This is comfortable
headroom for one full scrape, but the seven-day capacity alert remains active
and this phase intentionally did not start the worker.
...
| Final transaction | Finite relation/statement timeouts plus PostgreSQL 17 `transaction_timeout` enforce one cumulative remaining budget across statements and inner relation-lock attempts. Advisory-busy outcomes may retry before cutover starts; a cutover deadline now defers the ready generation immediately instead of resetting another five-second budget in the worker outer loop. `57014` cancellation fails immediately | Enforced `<=5s`; repaired after scrape `1279` exposed two outer deadline retries |
...
| Contention semantics | Advisory-busy outcomes retry the same preparation under a bounded policy. A final-cutover deadline immediately preserves `ready`, records `publication-commit-deferred`, blocks new scrape allocation, and permits a later deferred recovery pass without resetting the current cutover budget | Accepted in contention exhaustion/preservation/retry and single-deadline-deferral tests |
...
- commit intent from `06:15:57.194Z` until publication at `06:16:08.167Z`,
  ...
  recovery;
- the deployed B image retried two separate five-second cutover deadline
  failures before the successful `2.886s` attempt. The worker now treats a
  cutover deadline as immediate deferral, while retaining retries only for
  advisory-busy outcomes that occur before exclusive cutover starts;
...
| Repeated partials | A rejected >10% eviction retains the last exact baseline/token and skips partial in-memory mutation, so consecutive identical partial responses remain rejected and cannot later persist a truncated exact catalog | Accepted |
| Test fixture parity | Scrape-backed persistence/export/projection/retention fixtures retrieve the exact live catalog token and allocate through `StartScrapeRun(token)`, retaining catalog generations/bindings instead of bypassing production invariants with direct `scrape_log` inserts | Accepted |
...
| Exact inventory | 18 leaf tables; after secondary-index retirement, 20 retained primary-key indexes and `123,173,888,000` bytes | Pass |
...
| Executed reclaim | Rollback rehearsal plus short-timeout two-parent truncate, no `CASCADE`; rows zero, metrics/constraints intact, immediate and 60-second `13/13` public parity | **Accepted** |

The separate maintenance phase truncated only the two partitioned parents
without `CASCADE`; schemas, primary keys, and metrics remain. Database size
fell by `123,173,593,088` bytes and stable free space reached about
`164.33 GB`.
...
about `41.6-60.2 GB` practical build headroom while retaining rollback.
LOGICAL-RETIRE completed that prerequisite and created raw one-run space, but
the seven-day optional-build/rewrite guard still blocks a production shadow
build. A dedicated dual-build capacity model and one full
scrape/publication A/B remain required before any compact read-source cutover
or old projection drop.
...
- The bounded Duets pilot copied `4,651,508` rows with zero bidirectional
  differences and projected `62.87%` Duets reduction. Matched compact and v2
  warmed command p50/p95 tied; the compact cold SQL plan was faster.
- Retain all dates. The public route accepts 3,650 days and exports consume
  history; current band-type partitioning means row deletes do not return
  filesystem space.
...
> [!IMPORTANT]
> **Archived execution journal.** Completed, rejected, and future work are mixed
> below. Current data priorities live in
> [`docs/roadmap/data.md`](../../../roadmap/data.md).
