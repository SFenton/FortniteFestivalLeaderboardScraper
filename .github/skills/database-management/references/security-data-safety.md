...
FST destructive data/reclaim work requires matching live-scrape A/B evidence and fresh exact-object operator authorization through the destructive-maintenance machine. Parity is necessary evidence, never authorization. Before execution, bind the current publication, exact objects/actions, rollback or restore path, resource/disk risk, monitoring, and post-action validation.
