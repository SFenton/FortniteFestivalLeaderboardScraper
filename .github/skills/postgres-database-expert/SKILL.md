...
6. Avoid broad table rewrites, `VACUUM FULL`, column drops, non-concurrent large index builds, and destructive pruning until live-scrape A/B parity and fresh exact-object operator authorization through the destructive-maintenance machine are both present. Parity alone never approves execution; bind rollback and post-action validation.
