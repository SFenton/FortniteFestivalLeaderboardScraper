...
agents: [web-design-player, web-principal-architect, web-principal-designer, web-state]
...
5. Update `/memories/repo/web/features/player.md` after work

## Diagnostic Protocol

When investigating a player display issue or answering "why does X show/not show?":

1. **Check real data** — Use `fst-production/*` tools (`fst_player`, `fst_player_stats`, `fst_player_history`) or `web` tool to verify API responses
2. **Trace the data flow** — API response → `useTrackedPlayer` → component props → rendering
3. **Check data presence guards** — Verify that components handle missing/null player data gracefully
...
4. The calling orchestrator is responsible for invoking `web-design-player` to validate the result
